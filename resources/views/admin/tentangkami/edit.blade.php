...
<body class="bg-latar text-black pb-12 min-h-screen">
    ...
    <div class="fixed left-40 sm:left-56 bottom-16 z-10">
        <a href="{{ route('admin.tentangkami.detail', $post->tentang_id) }}" class="bg-nav rounded-r-md hover:bg-gradb text-xs md:text-base text-white py-2 px-4 md:px-8
            ...
            <div class="px-5 pt-20 md:px-20 w-full ">
                ...
                <div class="flex flex-col justify-between py-4 leading-normal">
                    ...
                    <form method="POST" action="{{ route('admin.tentangkami.update', $post->tentang_id) }}" enctype="multipart/form-data">
                        ...
                        <div>
                            <input name="title" value="{{ $post->title }}" type="text" class="w-full mt-5 border-black rounded-lg" placeholder="Edit Judul..">
                        </div>
                        @error('title')
                        <small  class=" text-red-700">{{ $message }}</small>
                        @enderror
